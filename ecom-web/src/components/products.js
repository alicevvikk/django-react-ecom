...
function Products() {
  ...
  const filtered = useMemo(() => {
    return products.filter(product => {
      if (filterCategory.toLowerCase() === 'all') return filterText.length > 0 ? product.name.toLowerCase().includes(filterText.toLowerCase()) && product.price < filterPrice: product.price < filterPrice;
      else if (product.kind.toLowerCase() === filterCategory.toLowerCase()) return filterText.length > 0 ? product.name.toLowerCase().includes(filterText.toLowerCase()) && product.price < filterPrice: product.price < filterPrice;
      // just in case 
